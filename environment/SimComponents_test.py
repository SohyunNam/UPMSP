...
class Source:
    def __init__(self, name, env, routing, monitor, jt_dict, p_j, K):
        ...
        self.iat = list(np.random.exponential(self.p_j, size=len(self.jobs)))
    ...
    def generate(self):
        while len(self.jobs):
            ...
            dd = self.due_date.pop(0)
            job.due_date = dd
            self.monitor.record(time=self.env.now, jobtype=job.job_type, event="Created", job=job.name, memo=job.due_date)
...
class Routing:
    ...
    def WSPT(self, location="Source", idle=None, job=None):
        if location == "Source":  # job -> machine 선택 => output : machine index
            ...
            for idx in range(len(idle)):
                if idle[idx]:
                    ...
                    if wpt < min_processing_time:
                        ...
                        min_machine_idx = idx
            self.monitor.record(time=self.env.now, jobtype=job.job_type, event="WSPT Determined",
                                machine=min_machine_idx)
            ...
            self.monitor.record(time=self.env.now, jobtype=next_job.job_type, event="WSPT Determined",
                                machine=location, memo=memo_list)
            return next_job
    ...
    def WMDD(self, location="Source", idle=None, job=None):
        if location == "Source":  # job -> machine 선택 => output : machine index
            ...
            self.monitor.record(time=self.env.now, jobtype=job.job_type, event="WMDD Determined",
                                machine=min_machine_idx)
            return min_machine_idx
            ...
            self.monitor.record(time=self.env.now, jobtype=next_job.job_type, event="WMDD Determined",
                                machine=location)
            return next_job
    ...
    def WCOVERT(self, location="Source", idle=None, job=None):
        ...
        if location == "Source":  # job -> machine 선택 => output : machine index
            ...
            self.monitor.record(time=self.env.now, jobtype=job.job_type, event="WCOVERT Determined",
                                machine=max_machine_idx)
            ...
            self.monitor.record(time=self.env.now, jobtype=next_job.job_type, event="WCOVERT Determined",
                                machine=location)
            return next_job
...
class Sink:
    ...
    def put(self, job):
        ...
        tardiness = self.env.now - job.due_date
